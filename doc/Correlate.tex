...
\publishers{\textcolor{blue}{This document can be found in your scripts directory, subfolder \texttt{/doc}}\\ \vspace{2mm} \small{corresponds to revision \svnrev\; commited at \svndate}\\\small{ of \texttt{martin\_correlate.m} at 
\svnloc}}
...
Correlation from a low resolution EM tomogram slice to a high magnification image of the same sample containing indentifiable fiducial markers such as the gold beads used for tomogram reconstruction is performed using the script \texttt{john\_manualregister\_LMtoHMtomo3.m}.
\subsection{Executing the script}
To execute the script and start the correlation simply run \begin{verbatim}
