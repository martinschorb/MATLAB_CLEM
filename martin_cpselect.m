function varargout = martin_cpselect(varargin)
...
%   $Revision: 1.1.6.1 $  $Date: 2006/11/08 17:49:20 $
...
cd '/g/software/linux/pack/matlab-7.4/toolbox/images/imuitools/private'
...
hflow = uiflowcontainer('Parent',hFig,...
                ...
                hgs = uicontrol('Style','slider','Position',[10,10,120,15]);    
...
activateAddPointMode() %#ok function handle evaluation
...
if args.Wait 
  ...
  %----------------------------------------
  function makeDefaultModeCurrent(varargin) %#ok varargin needed by HG caller
  ...
  %---------------------------------------
  function makeZoomInModeCurrent(varargin) %#ok varargin needed by HG caller
  ...
  %----------------------------------------
  function makeZoomOutModeCurrent(varargin) %#ok varargin needed by HG caller
  ...
  %------------------------------------
  function makePanModeCurrent(varargin) %#ok varargin needed by HG caller
  ...
  %------------------------------------
  function makeAddModeCurrent(varargin) %#ok varargin needed by HG caller
  ...
  %-------------------------------------------
  function makeAddPredictModeCurrent(varargin) %#ok varargin needed by HG caller
  ...
  %--------------------------------
  function deleteTool(varargin) %#ok varargin needed by HG caller
  ...
  %------------------------------------------
  function okPressed = exportPoints(varargin) %#ok varargin needed by HG caller
    ...
    [dummy,okPressed] = ...
        export2wsdlg(checkboxlabels,defaultvarnames,items2export,...
                     exportTitle,selected); %#ok dummy unused
    
  end

  %-------------------------------
  function closeCpselect(varargin) %#ok varargin needed by HG caller
  ...
  %--------------------------------------
  function closeRequestCpselect(varargin) %#ok varargin needed by HG caller
  ...
  %----------------------------------------
  function toggleShowOverview(src,varargin) %#ok varargin needed by HG caller
...
%-----------------------------------
function showCPSTHelp(varargin) %#ok varargin needed by HG caller

  ipthelp('cpselect_gui','Control Point Selection Tool');
